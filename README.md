...
跟其它程序设计语言一样，Bash中的条件语句让我们可以决定一个操作是否被执行。是否执行的结果是由执行一个包在`[[ ]]`里的表达式得出的。

条件表达式可以包含`&&`和`||`运算符，分别对应 _与_ 和 _或_ 。除此之外还有很多有用的[表达式](#TODO)。
...
在每次循环的过程中，`arg`依次被赋值为从`elem1`到`elemN`。arg的可选值还可以是通配符或者[大括号扩展](#TODO)。
...
函数可以接收参数并返回结果 —— 返回值。参数，在函数内部，跟[非交互式](#TODO)下的脚本参数处理方式相同 —— 使用[位置参数](#TODO)。返回值可以使用`return`命令 _返回_ 。
...
这本小册子讲述了我自己的Bash经验。它的本意并非全面综合，因此如果你想了解更多，请运行`man bash`，从那里开始。

非常欢迎您的贡献，任何指正和问题我都非常感激。这些都可以通过创建一个[issue
](#TODO)来进行。
